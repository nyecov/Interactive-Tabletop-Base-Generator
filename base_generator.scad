...
base_height_mm = 4.0; // [1.0:0.1:20.0]
...
module rounded_tapered_polygon_layer(h, r1, r2, corner_r) {
    if (h > 0) {
        // Adjust radii to account for corner rounding
        // r here is the circumradius of the base vertices (including corner_r)
        // so the hull points are at r - corner_r
        r1_adj = r1 - corner_r;
        r2_adj = r2 - corner_r;
        
        hull() {
            for (i = [0 : poly_sides - 1]) {
                angle = i * 360 / poly_sides;
                
                // Bottom corner cylinder
                translate([r1_adj * cos(angle), r1_adj * sin(angle), 0])
                cylinder(r = corner_r, h = 0.01, $fn = 24);
                
                // Top corner cylinder
                translate([r2_adj * cos(angle), r2_adj * sin(angle), h - 0.01])
                cylinder(r = corner_r, h = 0.01, $fn = 24);
            }
        }
    }
